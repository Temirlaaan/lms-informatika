...
export default function QuizResultPage() {
  ...
  const gradeColor =
    grade === 5 ? 'text-green-600 dark:text-green-400' :
    grade === 4 ? 'text-blue-600 dark:text-blue-400' :
    grade === 3 ? 'text-yellow-600 dark:text-yellow-400' : 'text-red-600 dark:text-red-400';
        ...
        {result.answers.map((a, idx) => (
          <div key={a.id} className={`bg-card rounded-xl shadow-sm p-5 border-l-4 ${
            ...
            <div className="flex justify-between items-start mb-3">
              ...
              <span className={`text-sm font-medium ${a.is_correct ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
